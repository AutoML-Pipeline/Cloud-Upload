      ...
      const handler = (event) => {
        ...
        if (event.data && (event.data.google_creds || event.data.access_token)) {
          ...
          if (accessToken) {
            ...
            alert("Google OAuth message received but no access token found. Check console.");
          }
          popup && popup.close();
    ...
    <div className="page-fullscreen">
      <div style={{ minHeight: '100vh', width: '100vw', overflow: 'hidden', position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', minHeight: 'calc(100vh - 54px)', zIndex: 10 }}>
          <div style={{ width: 420, maxWidth: '96vw', background: "rgba(30,41,59,0.93)", borderRadius: 18, boxShadow: "0 4px 24px rgba(99,102,241,0.13)", padding: "2.5rem 2rem", color: "#e0e7ef", minHeight: 360, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
