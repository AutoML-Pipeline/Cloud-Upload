...
// Unified FE job hook with analyze + apply support
// Contract:
// - startAnalyze(filename): Promise<{ step_recommendations, data_quality_notes }>
// - startApply(filename, steps)
...
export default function useFEJob({ notify } = {}) {
  ...
  // Analyze is not a background job; we expose it as a Promise-returning call
  const startAnalyze = useCallback(async (filename) => {
    if (!filename) {
      const err = new Error("Please select a dataset before analyzing.");
      notify?.error?.(err.message);
      throw err;
    }
    try {
      const res = await fetch(`http://localhost:8000/api/feature-engineering/analyze/${encodeURIComponent(filename)}`);
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data?.detail || data?.error || "Failed to fetch recommendations");
      // Normalize keys for consumers
      return {
        step_recommendations: data.step_recommendations || data.recommendations || [],
        data_quality_notes: data.data_quality_notes || data.quality_notes || [],
      };
    } catch (e) {
  ...
  const startApply = useCallback(async (filename, stepsPayload, { onBeforeStart } = {}) => {
    if (!filename) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data?.detail || data?.error || "Failed to start feature engineering job");
      if (!data?.job_id) throw new Error("Failed to obtain feature engineering job identifier");

      setJobId(data.job_id);
      setStatus("pending");
      setProgress(0);
      setMessage("Initializing feature engineering pipeline...");
      ...
      notify?.error?.(`Failed to start feature engineering: ${e.message}`);
    }
  }, [notify, clearPolling, beginTimer, clearTimer]);

  // Polling for apply job
  useEffect(() => {
    if (!jobId) {
      clearPolling();
      return;
    ...
    const poll = async () => {
      try {
        ...
        if (!res.ok) throw new Error(payload?.detail || payload?.error || "Unable to fetch feature engineering status");
        if (cancelled) return;
  ...
  setMessage((prev) => (payload.message ?? prev));
        ...
        if (payload.status === "completed") {
          ...
          setStatus("completed");
          setProgress(100);
          setMessage(payload.message || "Feature engineering complete");
          setJobId(null);
          const startedAt = startTimeRef.current;
          clearTimer();
          if (startedAt) setElapsedTime(Date.now() - startedAt);
          ...
          setMessage(payload.message || "Feature engineering failed");
          setError(payload.error || "Feature engineering failed");
          setJobId(null);
          const startedAt = startTimeRef.current;
          clearTimer();
          if (startedAt) setElapsedTime(Date.now() - startedAt);
  ...
  // Auto-clear transient progress banner after completion/failure
  useEffect(() => {
  ...
  return {
    ...
    startApply,
    reset,
    // also expose setter for result for save/cleanup flows
    setResult,
    message,
  };
}
